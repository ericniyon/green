                        ...
                        <div class="row align-items-center">
                            <div class="col-lg-4">
                                <h5>
                                    ...
                                    <span
                                        class="badge badge-soft-dark radius-50 fz-12 ml-1">{{ $withdraw_req->total() }}</span>
                                </h5>
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex gap-3 justify-content-lg-end">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a class="dropdown-item"
                                            href="{{ route('seller.delivery-man.withdraw-list-export') }}">Excel</a></li>
                                ...
                                <select name="delivery_withdraw_status_filter" onchange="status_filter(this.value)"
                                    ...
                                    <option value="all"
                                        {{ session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'all' ? 'selected' : '' }}>
                                        {{ \App\CPU\translate('All') }}</option>
                                    <option value="approved"
                                        {{ session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'approved' ? 'selected' : '' }}>
                                        {{ \App\CPU\translate('Approved') }}</option>
                                    <option value="denied"
                                        {{ session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'denied' ? 'selected' : '' }}>
                                        {{ \App\CPU\translate('Denied') }}</option>
                                    <option value="pending"
                                        {{ session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'pending' ? 'selected' : '' }}>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if (isset($wr->delivery_men))
                                                <span
                                                    class="title-color hover-c1">{{ $wr->delivery_men->f_name . ' ' . $wr->delivery_men->l_name }}</span>
                                            @else
                                        ...
                                        <td class="text-center">
                                            @if ($wr->approved == 0)
                                                <label
                                                    class="badge badge-soft-primary">{{ \App\CPU\translate('Pending') }}</label>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                ...
                                                @else
                                                    <a class="btn btn-outline-info btn-sm square-btn disabled"
                                                        href="#">
                                                        <i class="tio-invisible"></i>
                                                    </a>
                                                @endif
