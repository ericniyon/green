                    ...
                    <div class="card-body">
                        <div class="row gy-2">
                            <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                                ...
                                <span>{{ \App\CPU\translate('deliveryman_name') }} :
                                    {{ $order->delivery_man != null ? $order->delivery_man->f_name . ' ' . $order->delivery_man->l_name : \App\CPU\translate('not_assigned') }}</span>
                                <span>{{ \App\CPU\translate('deliveryman_email') }} :
                                    {{ $order->delivery_man != null ? $order->delivery_man->email : \App\CPU\translate('not_found') }}</span>
                                <span>{{ \App\CPU\translate('deliveryman_phone') }} :
                                    {{ $order->delivery_man != null ? $order->delivery_man->phone : \App\CPU\translate('not_found') }}</span>
                            </div>

                            <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                                <span>{{ \App\CPU\translate('payment_method') }} :
                                    {{ str_replace('_', ' ', $order->payment_method) }}</span>
                                <span>{{ \App\CPU\translate('order_details') }} : <a class="btn btn--primary btn-sm"
                                        href="{{ route('seller.orders.details', ['id' => $order->id]) }}">{{ \App\CPU\translate('click_here') }}</a></span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('seller.refund.refund-status-update') }}" id='order_place' method="post"
                        class="row">
                        @csrf
