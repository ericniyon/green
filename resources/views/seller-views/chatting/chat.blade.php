                        ...
                        <div class="inbox_people">
                            <form class="search-form px-3" id="chat-search-form">
                                <div class="search-input-group">
                                    ...
                                    <input class="" id="myInput" type="text"
                                        @if (Request::is('seller/messages/chat/customer')) placeholder="{{ \App\CPU\translate('Search_customers...') }}"
                                        @elseif(Request::is('seller/messages/chat/delivery-man'))
                                            placeholder="{{ \App\CPU\translate('Search_delivery_men...') }}" @endif
                                ...
                                @foreach ($chattings_user as $key => $chatting)
                                    <div class="list_filter">
                                        <div class="chat_list p-3 d-flex gap-2 user_{{ $chatting->user_id ? $chatting->user_id : $chatting->delivery_man_id }} seller-list @if ($key == 0) active @endif"
                                            id="{{ $chatting->user_id ? $chatting->user_id : $chatting->delivery_man_id }}"
                                            data-name="{{ $chatting->f_name }} {{ $chatting->l_name }}"
                                            data-phone="{{ $chatting->phone }}">
                                            <div class="chat_people media gap-10" id="chat_people">
                                                <div class="chat_img avatar avatar-sm avatar-circle">
                                                    <img @if (Request::is('seller/messages/chat/customer')) src="{{ asset('storage/profile/' . $chatting->image) }}"
                                                        @else
                                                            src="{{ asset('storage/delivery-man/' . $chatting->image) }}" @endif
                                                        id="{{ $chatting->user_id ? $chatting->user_id : $chatting->delivery_man_id }}"
                                                    ...
                                                    <span
                                                        class="avatar-satatus avatar-sm-status avatar-status-success"></span>
                                                </div>
                                                <div class="chat_ib media-body">
                                                    <h5 class="mb-1 seller @if ($chatting->seen_by_seller) active-text @endif"
                                                        ...
                                                        <br><span class="mt-2 font-weight-normal text-muted"
                                                            id="{{ $chatting->user_id ? $chatting->user_id : $chatting->delivery_man_id }}"
                                                            data-name="{{ $chatting->f_name }} {{ $chatting->l_name }}"
                                                            data-phone="{{ $chatting->phone }}">{{ $chatting->phone }}</span>
                                                    </h5>
                                                </div>
                                            </div>
                                            @if ($chatting->seen_by_seller == false)
                                                <div class="message-status bg-danger"
                                                    id="notif-alert-{{ $chatting->user_id ? $chatting->user_id : $chatting->delivery_man_id }}">
                                                </div>
                                            @endif
                                        </div>
                            ...
                            <div class="media align-items-center gap-3">
                                <div class="avatar avatar-sm avatar-circle">
                                    <img class="avatar-img" id="profile_image"
                                        @if (Request::is('seller/messages/chat/customer')) src="{{ asset('storage/profile/' . $chattings_user[0]->image) }}"
                                         @else
                                         src="{{ asset('storage/delivery-man/' . $chattings_user[0]->image) }}" @endif
                                        onerror="this.src='{{ asset('public/assets/front-end/img/image-place-holder.png') }}'"
                                        alt="Image Description">
                                    <span class="avatar-status avatar-sm-status avatar-status-success"></span>
                                </div>
                                <div class="media-body">
                                    <h5 class="profile-name mb-1" id="profile_name">
                                        {{ $chattings_user[0]->f_name . ' ' . $chattings_user[0]->l_name }}</h5>
                                            ...
                                            @else
                                                <div class="outgoing_msg">
                                                    <div class="sent_msg">
                                                        ...
                                                        <span class="time_date fz-12 d-flex justify-content-end">
                                                            {{ $message->created_at->format('h:i A') }} |
                                                            {{ $message->created_at->format('M d') }} </span>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="input_msg_write">
                                            <form class="mt-4" id="myForm">
                                                ...
                                                <input type="text" id="hidden_value" hidden
                                                    value="{{ $last_chat->user_id ? $last_chat->user_id : $last_chat->delivery_man_id }}"
                                                    name="">

                                                <textarea class="form-control h-120" id="msgInputValue" type="text"
                                                    placeholder="{{ \App\CPU\translate('Send a message') }}" aria-label="Search"></textarea>
                                                <div class="mt-3 d-flex justify-content-end">
                                                    <button class="aSend btn btn--primary" type="submit"
                                                        id="msgSendBtn">{{ \App\CPU\translate('Send_Reply') }}</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
    ...
    <script>
                ...
                success: function(data) {
                    ...
                    if (data.length != 0) {
                        data.map((element, index) => {
                            ...
                            let date = month[dateTime.getMonth().toString()] + " " + dateTime.getDate()
                                .toString();
        ...
        $(document).ready(function() {
            ...
            $("#msgSendBtn").click(function(e) {
                ...
                if ("{{ Request::is('seller/messages/chat/customer') }}" == true) {
                    post_url = "{{ route('seller.messages.ajax-seller-message-store') }}" + "?user_id=" +
                        user_id;
                } else if ("{{ Request::is('seller/messages/chat/delivery-man') }}" == true) {
                    post_url = "{{ route('seller.messages.ajax-seller-message-store') }}" +
                ...
                $.ajax({
                    ...
                    success: function(respons) {
                        ...
                        let time = dateTime.toLocaleTimeString().toLowerCase();
                        let date = month[dateTime.getMonth().toString()] + " " + dateTime
                            .getDate().toString();
