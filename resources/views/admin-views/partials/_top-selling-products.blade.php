                    ...
                    <div class="cursor-pointer"
                        ...
                        <div class="grid-item px-0 bg-transparent">
                            <div class="d-flex gap-10">
                                <img src="{{ \App\CPU\ProductManager::product_image_path('thumbnail') }}/{{ $item->product['thumbnail'] }}"
                                    class="avatar avatar-lg rounded avatar-bordered"
                                    onerror="this.src='{{ asset('assets/back-end/img/160x160/img2.jpg') }}'"
                                    alt="{{ $item->product->name }} image">
                                <div class="title-color">{{ substr($item->product['name'], 0, 20) }}
                                    {{ strlen($item->product['name']) > 20 ? '...' : '' }}</div>
                            </div>
