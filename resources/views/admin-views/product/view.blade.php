                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form class="form-group"
                                    action="{{ route('admin.product.deny', ['id' => $product['id']]) }}" method="post">
                ...
                <div class="row align-items-md-center gx-md-5">
                    ...
                    <div class="col-lg-4 mb-5 mb-lg-0 d-flex flex-column gap-1">
                        ...
                        <div class="flex-start">
                            ...
                            <span
                                class="mx-1">{{ $product->discount_type == 'flat' ? \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($product['discount'])) : $product->discount . '' . '%' }}
                            </span>
                        </div>
                        @if ($product->product_type == 'physical')
                            <div class="flex-start">
                                ...
                                <span
                                    class="mx-1">{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($product->shipping_cost)) }}</span>
                            </div>
                            <div class="flex-start">
                        ...
                        @if ($product->product_type == 'digital' && $product->digital_product_type == 'ready_product')
                            <div>
                                <a href="{{ asset("storage/app/public/product/digital-product/$product->digital_file_ready") }}"
                                    class="btn btn--primary py-1 mt-3" download>{{ \App\CPU\translate('download') }}</a>
                            </div>
                        @endif
                                ...
                                <div class="d-flex align-items-center flex-wrap gap-10 mb-4">
                                    <div class="">
                                        <div class="product-description-label">
                                            {{ \App\CPU\translate('Available color') }}:
                                        </div>
                                    </div>
                                    <div class="">
                                            ...
                                            @foreach (json_decode($product->colors) as $key => $color)
                                                <li>
                                                    ...
                                                    <label class="d-block p-2" style="background: {{ $color }};"
                                                        for="{{ $product->id }}-color-{{ $key }}"></label>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
