                            ...
                            <tr>
                                <td>
                                    <div class="media align-items-center gap-10">
                                        <img class="avatar avatar-sm"
                                            src="{{ asset('storage/product/thumbnail') }}/{{ $cartItem['image'] }}"
                                            onerror="this.src='{{ asset('assets/back-end/img/160x160/img2.jpg') }}'"
                                            alt="{{ $cartItem['name'] }} image">
                                        <div class="media-body">
                                            <h5 class="text-hover-primary mb-0">{{ Str::limit($cartItem['name'], 12) }}
                                            </h5>
                                ...
                                <td>
                                    <input type="number" data-key="{{ $key }}" class="form-control qty"
                                        ...
                                        onkeyup="updateQuantity('{{ $cartItem['id'] }}',this.value,event,'{{ $cartItem['variant'] }}')">
                                </td>
                                <td>
                                    <div>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        <a href="javascript:removeFromCart({{ $key }})"
                                            class="btn btn-sm btn-outline-danger square-btn"> <i
    ...
    <div class="pt-4">
        <dl>
            <div class="d-flex gap-2 justify-content-between">
                <dt class="title-color text-capitalize font-weight-normal">{{ \App\CPU\translate('sub_total') }} :
                </dt>
                <dd>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($subtotal)) }}</dd>
            </div>

            <div class="d-flex gap-2 justify-content-between">
                <dt class="title-color text-capitalize font-weight-normal">{{ \App\CPU\translate('product') }}
                    {{ \App\CPU\translate('discount') }} :</dt>
                <dd>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(round($discount_amount, 2))) }}
                </dd>
            </div>

            <div class="d-flex gap-2 justify-content-between">
                <dt class="title-color text-capitalize font-weight-normal">{{ \App\CPU\translate('extra') }}
                    {{ \App\CPU\translate('discount') }} :</dt>
                <dd>
            ...
            <div class="d-flex justify-content-between">
                <dt class="title-color gap-2 text-capitalize font-weight-normal">{{ \App\CPU\translate('coupon') }}
                    {{ \App\CPU\translate('discount') }} :</dt>
                <dd>
            ...
            <div class="d-flex gap-2 justify-content-between">
                ...
                <dd>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(round($total_tax_amount, 2))) }}
                </dd>
            </div>

            <div class="d-flex gap-2 border-top justify-content-between pt-2">
                <dt class="title-color text-capitalize font-weight-bold title-color">{{ \App\CPU\translate('total') }}
                    : </dt>
                <dd class="font-weight-bold title-color">
                    {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(round($total + $total_tax_amount - $coupon_discount, 2))) }}
                </dd>
            </div>
        </dl>
        <form action="{{ route('admin.pos.order') }}" id='order_place' method="post">
            ...
            <div class="pt-4 mb-4">
                ...
                <ul class="list-unstyled option-buttons">
                    <li>
                        ...
                        <label for="cash"
                            class="btn btn--bordered btn--bordered-black px-4 mb-0">{{ \App\CPU\translate('cash') }}</label>
                    </li>
                    <li>
                        <input type="radio" value="card" id="card" name="type" hidden>
                        <label for="card"
                            class="btn btn--bordered btn--bordered-black px-4 mb-0">{{ \App\CPU\translate('Card') }}</label>
                    </li>
                </ul>
            </div>
            <div class="d-flex gap-2 justify-content-between align-items-center pt-3">
                ...
                <button id="submit_order" type="button" onclick="form_submit()" class="btn btn--primary btn-block m-0"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <button class="btn btn--primary" onclick="extra_discount();"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button id="coupon_close" type="button" class="close" data-dismiss="modal"
                        aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
