                    ...
                    <div class="card-body">
                        <div class="row gy-2">
                            <div class="col-sm-4 col-md-4 col-lg-2">
                                <div>
                                    <img onerror="this.src='https://res.cloudinary.com/dxuoorngr/image/upload/v1710758318/image-place-holder_cdnntk.png'"
                            ...
                            <div class="col-sm-8 col-md-4 col-lg-6">
                                ...
                                <div class="mb-1">{{ \App\CPU\translate('QTY') }} : {{ $refund->order_details->qty }}
                                </div>
                                <div class="mb-1">{{ \App\CPU\translate('price') }} :
                                    {{ \App\CPU\Helpers::currency_converter($refund->order_details->price) }}</div>
                        ...
                        <div class="d-flex flex-wrap flex-column flex-md-row gap-10 justify-content-between">
                            <span class="title-color">{{ \App\CPU\translate('subtotal') }} :
                                {{ \App\CPU\Helpers::currency_converter($subtotal) }}</span>
                            <span class="title-color">{{ \App\CPU\translate('coupon_discount') }} :
                                {{ \App\CPU\Helpers::currency_converter($coupon_discount) }}</span>
                            <span class="title-color">{{ \App\CPU\translate('total_refund_amount') }} :
                                {{ \App\CPU\Helpers::currency_converter($refund_amount) }}</span>
                        </div>
                        ...
                        <div class="row gy-2">
                            <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                                ...
                                @if ($order->seller_is == 'seller' && $order->seller != null)
                                    ...
                                    <div>{{ \App\CPU\translate('seller_email') }} : <a class="text-dark"
                            ...
                            <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                                ...
                                <div>{{ \App\CPU\translate('deliveryman_name') }} :
                                    {{ $order->delivery_man != null ? $order->delivery_man->f_name . ' ' . $order->delivery_man->l_name : \App\CPU\translate('not_assigned') }}
                                </div>
                                <div>{{ \App\CPU\translate('deliveryman_email') }} :
                                    {{ $order->delivery_man != null ? $order->delivery_man->email : \App\CPU\translate('not_found') }}
                                </div>
                                <div>{{ \App\CPU\translate('deliveryman_phone') }} :
                                    {{ $order->delivery_man != null ? $order->delivery_man->phone : \App\CPU\translate('not_found') }}
                                </div>
                            </div>

                            <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                                <div>{{ \App\CPU\translate('payment_method') }} :
                                    {{ str_replace('_', ' ', $order->payment_method) }}</div>
                                <div class="d-flex flex-wrap gap-2">{{ \App\CPU\translate('order_details') }} : <a
                                        class="btn btn--primary btn-sm"
                                        href="{{ route('admin.orders.details', ['id' => $order->id]) }}">{{ \App\CPU\translate('click_here') }}</a>
                                </div>
                    ...
                    <form action="{{ route('admin.refund-section.refund.refund-status-update') }}" id='order_place'
                        ...
                        <div class="form-group col-12">
                            ...
                            <select name="refund_status" class="form-control"
                                ...
                                <option value="refunded" {{ $refund->status == 'refunded' ? 'selected' : '' }}>
                ...
                if (result.value) {
                    ...
                    $.post({
                        ...
                        success: function(data) {
                            ...
                            if (data === 'success') {
                                toastr.success(
                                    '{{ \App\CPU\translate('refund_status_updated_successfully!!') }}!', {
                                        CloseButton: true,
                                        ProgressBar: true
                                    });
                            } else {
                                toastr.warning(
                                    '{{ \App\CPU\translate('it is already refunded, you can not change it!!') }}!', {
                                        CloseButton: true,
                                        ProgressBar: true
                                    });
                            }
