                        ...
                        <div class="d-flex flex-wrap gap-10 justify-content-between mb-4">
                            <div class="d-flex flex-column gap-10">
                                ...
                                <div class="d-flex flex-wrap gap-10">
                                    <div
                                        class="badge-soft-info font-weight-bold d-flex align-items-center rounded py-1 px-2">
                                        {{ \App\CPU\translate('linked_orders') }} ({{ $linked_orders->count() }}) : </div>
                            ...
                            @else
                                @if (!empty($order->seller->shop))
                                    <div class="mr-3">
                                        <img class="avatar rounded avatar-70"
                                            onerror="this.src='https://6valley.6amtech.com/public/assets/front-end/img/image-place-holder.png'"
                                            src="{{ asset('storage/shop') }}/{{ $order->seller->shop->image }}"
                                            alt="">
                                    </div>
                                    <div class="media-body d-flex flex-column gap-2">
                ...
                google.maps.event.addListener(marker, 'click', (function(marker) {
                    return function() {
                        infowindow.setContent(
                            "<div class='float-left'><img class='__inline-5' src='{{ asset('storage/profile/') }}{{ $order->customer->image ?? '' }}'></div><div class='float-right __p-10'><b>{{ $order->customer->f_name ?? '' }} {{ $order->customer->l_name ?? '' }}</b><br/>{{ $shipping_address->address ?? '' }}</div>"
                        );
