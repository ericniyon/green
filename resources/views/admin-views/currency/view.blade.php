            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">
                            <img width="18" src="{{ asset('/public/assets/back-end/img/currency.png') }}"
                                alt="">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <input type="text" name="name" class="form-control" id="name"
                                            placeholder="{{ \App\CPU\translate('Enter currency Name') }}">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <input type="text" name="symbol" class="form-control" id="symbol"
                                            placeholder="{{ \App\CPU\translate('Enter currency symbol') }}">
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <input type="text" name="code" class="form-control" id="code"
                                            placeholder="{{ \App\CPU\translate('Enter currency code') }}">
                                    </div>
                                        ...
                                        <select class="form-control js-select2-custom" name="currency_id">
                                            @foreach (App\Model\Currency::where('status', 1)->get() as $key => $currency)
                                                <option value="{{ $currency->id }}"
                                                    {{ $default->value == $currency->id ? 'selected' : '' }}>
                                                    {{ $currency->name }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="row align-items-center">
                            <div class="col-sm-4 col-md-6 col-lg-8 mb-2 mb-sm-0">
                                <h5 class="mb-0 d-flex align-items-center gap-10">
                                    ...
                                    <span
                                        class="badge badge-soft-dark radius-50 fz-12 ml-1">{{ $currencies->total() }}</span>
                                </h5>
                            </div>
                                ...
                                @foreach ($currencies as $key => $data)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($default['value'] != $data->id)
                                                <label class="switcher">
                                                    <input type="checkbox" class="switcher_input status"
                                                        id="{{ $data->id }}" <?php if ($data->status == 1) {
                                                            echo 'checked';
                                                        } ?>>
                                            ...
                                            @else
                                                <label
                                                    class="badge badge-info">{{ \App\CPU\translate('Default') }}</label>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="d-flex gap-10 justify-content-center">
                                                @if ($data->code != 'USD')
                                                    <a title="{{ \App\CPU\translate('Edit') }}" type="button"
                                                        class="btn btn-outline--primary btn-sm btn-xs edit"
                                                        href="{{ route('admin.currency.edit', [$data->id]) }}">
                                                        <i class="tio-edit"></i>
                                                    </a>
                                                    @if ($default['value'] != $data->id)
                                                        <a title="{{ \App\CPU\translate('Delete') }}" type="button"
                                                            class="btn btn-outline-danger btn-sm btn-xs delete"
                                                            id="{{ $data->id }}">
...
@push('script')
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.status == 1) {
                                toastr.success(
                                    '{{ \App\CPU\translate('Currency removed successfully!') }}'
                                    );
                                location.reload();
                            } else {
                                toastr.warning(
                                    '{{ \App\CPU\translate('This Currency cannot be removed due to payment gateway dependency!') }}'
                                    );
                                location.reload();
                            }
                        }
    ...
    <script>
        function default_currency_delete_alert() {
            toastr.warning(
                '{{ \App\CPU\translate('default currency can not be deleted!to delete change the default currency first!') }}'
                );
        }
    </script>
@endpush
