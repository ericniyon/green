...
<div class="navbar-tool dropdown {{ Session::get('direction') === 'rtl' ? 'mr-md-3' : 'ml-md-3' }}"
    ...
    <a class="navbar-tool-text {{ Session::get('direction') === 'rtl' ? 'mr-2' : 'ml-2' }}"
        href="{{ route('shop-cart') }}"><small>{{ \App\CPU\translate('my_cart') }}</small>
        ...
        <div class="widget widget-cart px-3 pt-2 pb-3">
            @if ($cart->count() > 0)
                    ...
                    @foreach ($cart as $cartItem)
                        <div class="widget-cart-item pb-2">
                            <button class="close text-danger " type="button"
                                onclick="removeFromCart({{ $cartItem['id'] }})" aria-label="Remove"><span
                            ...
                            <div class="media">
                                ...
                                <div class="media-body">
                                    ...
                                    <div class="widget-product-meta">
                                        <span
                                            class="text-muted {{ Session::get('direction') === 'rtl' ? 'ml-2' : 'mr-2' }}">x
                ...
                <a class="btn btn--primary btn-sm btn-block" href="{{ route('checkout-details') }}">
                    <i
                        class="czi-card {{ Session::get('direction') === 'rtl' ? 'ml-2' : 'mr-2' }} font-size-base align-middle"></i>{{ \App\CPU\translate('Checkout') }}
                </a>
            @else
                <div class="widget-cart-item">
                    <h6 class="text-danger text-center m-0"><i class="fa fa-cart-arrow-down"></i>
                        {{ \App\CPU\translate('Empty') }} {{ \App\CPU\translate('Cart') }}
                    </h6>
                </div>
            @endif
        </div>
