                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-4 col-sm-3 text-left d-flex flex-column pl-0 mt-2 mt-sm-0 pl-sm-5">
                                        ...
                                        <span>{{ \App\CPU\translate('price') }} :
                                            {{ \App\CPU\Helpers::currency_converter($order_details->price) }}</span>
                                        <span>{{ \App\CPU\translate('discount') }} :
                                            {{ \App\CPU\Helpers::currency_converter($order_details->discount) }}</span>
                                        <span>{{ \App\CPU\translate('tax') }} :
                                            {{ \App\CPU\Helpers::currency_converter($order_details->tax) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mt-2">
                            <div class="card-body">
                                <div class="row text-center">
                                    <span class="col-sm-2">{{ \App\CPU\translate('subtotal') }}:
                                        {{ \App\CPU\Helpers::currency_converter($subtotal) }}</span>
                                    <span class="col-sm-5">{{ \App\CPU\translate('coupon_discount') }}:
                                        {{ \App\CPU\Helpers::currency_converter($coupon_discount) }}</span>
                                    <span
                                        class="col-sm-5">{{ \App\CPU\translate('total_refundable_amount') }}:{{ \App\CPU\Helpers::currency_converter($refund_amount) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="card mt-2">
                                        ...
                                        <div class="col-12">
                                            <div class="form-group">
                                                <label class="input-label"
                                                    for="name">{{ \App\CPU\translate('refund_reason') }}</label>
                                                <textarea class="form-control" name="refund_reason" cols="120" required>{{ old('details') }}</textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-8">
                                            <div class="form-group">
                                                <label
                                                    for="exampleInputEmail1">{{ \App\CPU\translate('attachment') }}</label>
    ...
    <script type="text/javascript">
        $(function() {
            $(".coba").spartanMultiImagePicker({
                ...
                maxFileSize: '',
                placeholderImage: {
                    image: 'https://res.cloudinary.com/dxuoorngr/image/upload/v1710758318/image-place-holder_cdnntk.png',
                    width: '100%'
                },
