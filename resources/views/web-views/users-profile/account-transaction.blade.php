            ...
            <div class="row">
                ...
                <div class="col-md-9 sidebar_heading">
                    <h1 class="h3  mb-0 float-{{ Session::get('direction') === 'rtl' ? 'right' : 'left' }} headerTitle">
                        {{ \App\CPU\translate('purchase_statement') }}</h1>
                </div>
            </div>
        </div>
        <!-- Page Content-->
                        ...
                        <table class="table table-striped table-bordered">
                            <thead>
                                <tr style="background: {{ $web_config['secondary_color'] }}">
                                    <td class="tdBorder">
                                        <div class="py-2"><span
                                                class="d-block spandHeadO ">{{ \App\CPU\translate('Tranx') }}
                                                {{ \App\CPU\translate('ID') }}</span></div>
                                    </td>
                                    <td class="tdBorder">
                                    ...
                                    <td class="tdBorder">
                                        <div class="py-2"><span class="d-block spandHeadO">
                                                {{ \App\CPU\translate('Total') }}</span></div>
                                    </td>
                                </tr>
                            </thead>
                                ...
                                @foreach ($transactionHistory as $history)
                                    <tr>
                                        ...
                                        <td class="bodytr"><span
                                                class=" amount ">{{ \App\CPU\Helpers::currency_converter($history->order->order_amount) }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
